...
const essentialReducers = (state = converterReducers, action) => {
  switch (action.type) {
    case CONVERTER_REDUCER.SET_MONGO_THUNK:
      return state.set("vendor", fromJS(action?.uuid));
    case CONVERTER_REDUCER.SET_HISTORY_STORE:
      return state.update("transactions", (store) =>
        store.concat(action?.payload)
      );
    ...
    case CONVERTER_REDUCER.SET_AMOUNT:
      return state.setIn(
        ...
        fromJS(action?.value && Number(action?.value.amount))
      );
    case CONVERTER_REDUCER.SET_DATA_CONVERTER_VALUE:
      return state.setIn(
        ["amount", "converted_amount"],
        fromJS(action?.currency)
      );
